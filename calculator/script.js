document.addEventListener("DOMContentLoaded", function() {
    ...
    initHistory(history, historyList);
    ...
    function calculate() {
        ...
        if (isNaN(num1) || isNaN(num2)) {
            addHistoryItem(history, historyList, 'Введите корректные числа!', true, maxHistory);
            return;
        }
        ...
        switch (operation) {
            ...
            case '/':
                if (num2 === 0) {
                    addHistoryItem(history, historyList, 'Деление на 0 невозможно!', true, maxHistory);
                    return;
                }
                res = num1 / num2;
                break;
        ...
        addHistoryItem(history, historyList, expression, false, maxHistory);
    }
...
function updateHistoryUI(history, historyListElement) {
    historyListElement.innerHTML = '';
    if (history.length === 0) {
        historyListElement.textContent = 'История пуста';
        return;
    }
    ...
    history.forEach((item, index) => {
        ...
        historyListElement.appendChild(p);
    });
}

function initHistory(history, historyListElement) {
    ...
    updateHistoryUI(history, historyListElement);
}

function addHistoryItem(history, historyListElement, text, isError, maxHistory) {
